...
Implemented for the provider-backed holdings slice. Manual UI QA and richer pricing/history enrichment remain follow-on work.
...
- Live Alchemy portfolio responses now reconcile ERC-20 rows into the same scoped `TokenHolding` store using the existing read-only provider stack.
- The screen reads persisted holdings rows for the active scope and keeps cached native and ERC-20 state visible when a later refresh does not produce a fresh provider response.

## Follow-On Work

- Add pricing, valuation, and richer token identity fields only when the detail and dashboard consumers are ready.
...
- Follow-on pricing/history enrichment work is explicitly deferred rather than being implied complete.
