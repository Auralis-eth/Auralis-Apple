...
public final class AlchemyNFTService: NFTInventoryProviding, Sendable {
    ...
    public init(
        ...
        self.session = session ?? defaultAlchemyNFTSession
    ...
    public enum SpamConfidenceLevel: String, CaseIterable {
    ...
    public enum NFTFilter: String, CaseIterable {
        case spam = "SPAM"
    ...
    public func nftsForOwner(
        ...
        do {
            ...
            logger.notice("Primary owner fetch failed for current request; retrying degraded request")
            return try await degradedNFTsForOwner(owner: owner, pageKey: pageKey)
        }
    }

    public func getNFTsForOwner(
        owner: String,
