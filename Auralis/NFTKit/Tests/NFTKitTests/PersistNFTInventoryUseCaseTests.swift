...
struct PersistNFTInventoryUseCaseTests {
    ...
    func cleanupRemovesStaleInventoryAndPrunesOrphans() async throws {
        ...
        #expect(try #require(persistedNFTs.first).tokenId == "fresh")
        #expect(contracts.count == 1)
        #expect(try #require(contracts.first).address == "0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb")
        #expect(collections.count == 1)
        #expect(try #require(collections.first).contractAddress == "0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb")
    }
}
