...
struct NFTServiceTests {
    ...
    func sequencesCollaboratorsInOrder() async throws {
        ...
        let lastSuccessfulRefresh = try #require(
            service.lastSuccessfulRefreshAt(
                for: "0x1234567890abcdef1234567890abcdef12345678",
                chain: .ethMainnet
            )
        )
        #expect(lastSuccessfulRefresh.timeIntervalSince1970 > 0)
    }
    ...
    @Test(
        "same-scope refreshes reuse in-flight work",
        .timeLimit(.minutes(1))
    )
    @MainActor
    ...
    @Test(
        "changing scope cancels prior in-flight refreshes",
        .timeLimit(.minutes(1))
    )
    @MainActor
    ...
    @Test(
        "scope changes do not persist inventory prepared by a cancelled refresh",
        .timeLimit(.minutes(1))
    )
    @MainActor
