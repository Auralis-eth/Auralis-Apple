...
public enum RetryAfterSupport {
    public static func parse(_ header: String, now: Date = .now) -> TimeInterval? {
    ...
    public static func parse(from response: HTTPURLResponse, now: Date = .now) -> TimeInterval? {
