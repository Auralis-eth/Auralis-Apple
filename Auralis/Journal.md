...
- P0-452 routing constraint: Music detail navigation has one job now, and that job is to look like one navigation system. The user explicitly called out the classic SwiftUI footgun here: double back arrows from nested or competing stacks. For this ticket, item detail and collection detail must live inside the Music experience without making the UI feel like two coordinators are fighting over the same hallway.
- P0-452 item detail landed with a split-brain truce that actually makes sense: `NFT` stays the canonical owner for the richer identity story, while `MusicLibraryItem` acts like the fast local index card in the drawer. The important engineering lesson is that “canonical” and “best for browsing” are not always the same thing. Trying to force one model to do both jobs would have made the Music detail screen either too thin or too brittle.
- P0-452 collection detail forced the routing question into the open. The fix was not “add another little stack and hope nobody notices.” The fix was to give Music its own route type so item and collection detail could coexist on one stack. That is the durable lesson: when a feature branch starts needing different destinations, stop pretending a borrowed route enum is “close enough.” Model the route honestly before the back button starts telling on you.
- P0-452 validation closed with a very specific line in the sand: browsing and detail are in, but richer collection playback is still out. That distinction matters. Engineers get into trouble when a new detail screen starts smuggling in transport controls, queue semantics, and “while we’re here” playback logic. This slice proved the screens can open, degrade honestly, and share one Music stack without dragging the audio engine into a second unplanned war.
- P0-462 started with a nice reminder that tickets can lie after the code moves on. The docs still said “partially blocked,” but the repo had already crossed the line: the holdings list had a stable row model, the router had an ERC-20 detail route, and there was already a mounted placeholder detail screen. Translation: always re-check the seam in code before treating an old dependency note like scripture.
- P0-462 step 2 was a classic “replace the cardboard stand-in with a real screen” move. The placeholder detail view already proved the route existed; the real work was making the screen query the scoped local holding and admit when the data is thin. That is the recurring theme in this project: honesty beats fake richness. A detail screen that says “metadata is sparse” is much better than one that quietly fabricates completeness.
- P0-462 edge-case coverage reinforced a subtle point: even when a route currently opens only for ERC-20 rows, the presentation layer still benefits from understanding native-style fallback states. That is good engineering hygiene. You do not want your UI contract to explode the first time a later ticket broadens the entry conditions. Build the screen so it can speak clearly about incomplete reality now, and future tickets become additive instead of surgical.

- Phase 9 finally turned Search from a clever classifier into a real product surface. The important design choice was refusing to let Search become its own little destination world. Search now detects and routes, but the owned experiences still live in their home tabs: profile detail lives on the Profile tab, NFT collection and item detail live on the NFT tab, and token detail lives on the ERC-20 tab. That keeps the back-stack story honest and stops Search from quietly becoming a second app shell.
- The other useful lesson from Phase 9 is that “history” means commitment, not keystroke surveillance. The first history slice stores only committed searches, scopes them per active account, and makes recall cheap without turning the app into an analytics diary. That is the right bar for a wallet-shaped product: useful memory, low noise, minimal creepiness.
- The search testing gap turned out to be a familiar SwiftUI trap: the important behavior was real, but it lived as inline `if` branches inside `SearchRootView`, so the router and parser were tested while the screen-state contract itself was only being trusted by implication. The fix was not UI automation. The fix was to give the search root a tiny presentation brain of its own: history, safety, no-results, or results. Once that choice had a name, the tests could grab it directly and verify the product story without pretending snapshots are the only way to test a view.
- `P0-602` was a good example of a ticket that looked unfinished in docs but half-existed in code. There was already an Observe-only execution gate, but it still wore a narrow name and still assumed plugin execution should be blocked. The useful refactor was to tell the truth about the contract: this is a shared action-policy gate, not a one-off Observe helper, and not every controlled action is denied in Observe mode. Once that was explicit, the code could express the real product rule cleanly: signing, approvals, and transaction drafting are blocked; plugin/tool execution is still allowed because AI-heavy workflows are part of the app’s usable surface, not an afterthought.
- `P0-701B` reinforced a quieter architectural lesson: the most dangerous boundary leaks are not always dramatic. Sometimes they are tiny convenience constructions hiding in views, like a search screen spawning its own history store or an auth surface quietly building `AccountStore` on the spot. Those shortcuts feel harmless until they multiply. The first enforcement pass fixed the highest-value version of that pattern by making the mounted `ShellServiceHub` the one obvious kitchen door for shell-facing account, receipt, search-history, and token-holdings seams. That is the kind of cleanup that makes later smoke tests sharper: fewer mystery side doors, fewer “who owns this?” debates, and a much easier code-review story.
- `P0-702` was a nice reminder that safety language only works if normal people can read it without rolling their eyes. “Untrusted input” is accurate, but it is also mushy when splashed everywhere. The better contract was to keep the trust warning explicit while naming the thing: `Untrusted metadata`, `Untrusted link`, `Untrusted scan`, `Untrusted deep link`. That reads like a product decision instead of a compliance sticker. The first rollout hit the representative seams that actually matter: provider-fed NFT text, outbound links, scanned wallet content, and deep-link failures. Not every field in the app needs a warning label on day one, but the important seams now speak the same trust language.
- `P0-703` was the payoff for doing the earlier hardening tickets in the right order. Once the policy gate, boundary seams, and trust labels were real, the smoke tests could stay small and honest. That is the trick with smoke suites: they are not supposed to impersonate the whole app. They are supposed to sit at the front door with a flashlight and catch the obvious nonsense. In this case the flashlight checks four things: blocked actions really get denied with receipts, allowed plugin actions do not get falsely denied, raw deep-link input gets the trust warning while ordinary internal lookup failures do not, and Search still behaves like a launcher instead of quietly hoarding other tabs’ detail routes. Small surface area, high signal.
- `P0-502B` was the black-box-recorder cleanup pass, and the useful lesson was that payload hygiene beats cosmetic renaming almost every time. The receipt system already had decent trigger/scope structure. The real trust leak was simpler: mounted flows were still persisting raw outbound URLs and raw copied values. That is exactly how “helpful logging” quietly turns into “why did we save that?” The fix stayed disciplined. No schema rewrite. No new receipt categories. Just better sanitization for the fields that are genuinely sensitive on the current mounted path, plus tests proving that correlation IDs, account scope, chain provenance, export ordering, and reset behavior all stayed intact.
- That ticket also reinforced a subtle engineering habit: if a cleanup pass claims to improve trustworthiness, the tests should read like an auditor, not a stylist. The important checks were not “did we rename enough things?” They were “does the logger still preserve flow context after redaction?” and “does the store still behave like an append-only timeline after cleanup?” That is the difference between tidying code and hardening a product surface. One makes the diff look nicer. The other makes the app less likely to surprise you six tickets later.
